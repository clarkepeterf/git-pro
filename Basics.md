...
1. **Modified** - the file has been changed, but those changes have not been stored in the repository - these files show up as red 🔴 in `git status`
2. **Staged** - the file has been changed and is _marked_ to be committed - these files show up as green 🟢 in `git status`
3. **Committed** - the changes have been stored in the repository - committed files no longer show up in `git status`

#### Files can also be:

1. **Untracked** - a file not previously added and committed to the repository these files show up separately, and are also red 🔴 in git status
